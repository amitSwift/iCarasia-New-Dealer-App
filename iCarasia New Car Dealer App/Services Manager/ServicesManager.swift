...
class ServicesManager: NSObject {
    ...
    func getFilterReviews(parameters : NSMutableDictionary!,filterStr : String, completion: @escaping (_ result: NSDictionary , _ error : NSError? ) -> Void){
        
        let dealership_ID                   = parameters.value(forKey: "dealership_ID")
        let urlString: String               = BASE_URL + "dealer-api/dealership/3/reviews?" + "\(filterStr)"
        let urlWithPercentEscapes           = urlString.addingPercentEncoding( withAllowedCharacters: .urlQueryAllowed)
        let request: NSMutableURLRequest    = NSMutableURLRequest(url: NSURL(string: urlWithPercentEscapes!)! as URL)
        request.httpMethod                  = "GET"
